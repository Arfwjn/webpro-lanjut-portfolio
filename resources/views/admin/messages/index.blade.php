...
<div class="py-12">
    <div class="container mx-auto px-6 max-w-5xl">
        ...
        <div class="flex justify-between items-center mb-8 flex-wrap gap-4">
            ...
            @if ($messages->where('is_read', false)->count() > 0)
                <form method="POST" action="{{ route('admin.messages.read-all') }}">
                    @csrf
                    <button type="submit"
                            class="px-6 py-2 transition-all bg-emerald-500 hover:bg-emerald-600
                                   hover:shadow-lg hover:shadow-emerald-500/25 hover:-translate-y-0.5
                                   font-semibold rounded-xl text-white whitespace-nowrap text-sm">
                        Tandai Semua Dibaca
                    </button>
                </form>
            @endif
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-gray-100 dark:border-slate-700 overflow-hidden">
            @if ($messages->count() > 0)
                <div class="divide-y divide-gray-100 px-5 dark:divide-slate-700">
                    @foreach ($messages as $msg)
                        <div class="flex items-start gap-4 py-4 pr-2 transition-all duration-200
                            {{ !$msg->is_read
                                ? 'pl-7 border-l-4 border-indigo-500 bg-indigo-50 dark:bg-indigo-950/30'
                                : 'pl-7 border-l-4 border-transparent hover:bg-gray-50 dark:hover:bg-slate-700/50'
                            }}">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1 flex-wrap">
                                    <span class="{{ !$msg->is_read ? 'font-bold' : 'font-semibold' }} text-gray-900 dark:text-white">
                                        {{ $msg->name }}
                                    </span>
                            ...
                            <div class="flex-shrink-0 flex gap-2">
                                <a href="{{ route('admin.messages.show', $msg) }}"
                                   class="px-3 py-1.5 text-white rounded-lg font-medium text-xs
