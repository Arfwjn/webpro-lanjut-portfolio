...
<div class="py-20">
    <div class="container mx-auto px-6 max-w-6xl">
        <h1 class="font-lexend text-5xl font-bold text-center mb-16 bg-gradient-to-r from-emerald-500 to-emerald-600 bg-clip-text text-transparent leading-relaxed pb-2">
            Profiles
        </h1>
                        ...
                        @foreach ($profile->social_links as $platform => $link)
                            @if ($link)
                                <a href="{{ $link }}" target="_blank"
                                   class="px-3 py-1 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 rounded-full text-sm font-medium hover:bg-emerald-200 transition-colors">
                                    {{ ucfirst($platform) }}
                                </a>
                            @endif
                        @endforeach
